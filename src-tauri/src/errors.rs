...
impl AppError {
    ...
    pub fn user_message(&self) -> String {
        match self {
            AppError::Database(_) => "データベースエラーが発生しました。アプリケーションを再起動してください。".to_string(),
            AppError::BoothClient(_) => "BOOTH商品情報の取得に失敗しました。URLを確認してください。".to_string(),
            AppError::Network(_) => "ネットワークエラーが発生しました。インターネット接続を確認してください。".to_string(),
            AppError::Io(_) => "ファイル操作中にエラーが発生しました。ファイルの権限を確認してください。".to_string(),
            AppError::UrlParse(_) => "無効なURLです。正しいBOOTH URLを入力してください。".to_string(),
            AppError::ZipExtraction(_) => "ZIPファイルの展開に失敗しました。ファイルが破損している可能性があります。".to_string(),
